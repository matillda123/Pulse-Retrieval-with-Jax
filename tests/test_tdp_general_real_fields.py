from pulsedjax.real_fields import tdp #DifferentialEvolution, Evosax, LSF, AutoDiff
...
delay, frequency, trace, spectra = pulse_maker.generate_tdp(time, frequency, pulse_t, pulse_f, nonlinear_method="pg", spectral_filter=None, N=64, 
                                                             ...
                                                             cut_off_val=1e-1, frequency_range=(0,1), real_fields=True)
...
def test_DifferentialEvolution(parameters):
    ...
    de = tdp.DifferentialEvolution(delay, frequency, trace, nonlinear_method, cross_correlation=cross_correlation, f_range_fields=(0,1))
...
def test_Evosax(parameters):
    ...
    evo = tdp.Evosax(delay, frequency, trace, nonlinear_method, cross_correlation=cross_correlation, f_range_fields=(0,1))
...
def test_LSF(parameters):
    ...
    lsf = tdp.LSF(delay, frequency, trace, nonlinear_method, cross_correlation=cross_correlation, f_range_fields=(0,1))
...
def test_AutoDiff(parameters):
    ...
    ad = tdp.AutoDiff(delay, frequency, trace, nonlinear_method, cross_correlation=cross_correlation, f_range_fields=(0,1))
